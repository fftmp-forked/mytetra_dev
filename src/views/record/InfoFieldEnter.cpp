...
QString InfoFieldEnter::getField(QString name) {
    if (checkFieldName(name)) {
        ...
        if (name == "tags")
            return recordTags->text().simplified(); // При возврате значения, для тегов обязательно нужно убирать переносы строк, если они есть
    } else
...
void InfoFieldEnter::setField(QString name, QString value) {
    if (checkFieldName(name)) {
        ...
        if (name == "tags")
            recordTags->setText(value.simplified()); // При внешней установке значения, для тегов обязательно нужно убирать переносы строк, если они есть
    } else
